...
/* ============================================================
   DOWNSTREAM EMPLOYEE LIST
============================================================ */

type DownstreamEmployeeItem struct {
	EmployeeID   string `json:"employee_id"`
	EmployeeName string `json:"employee_name"`
	Designation  string `json:"designation"`
	Department   string `json:"department"`
...
/* ============================================================
   GET /api/v1/user/downstream-employees
   ...
   SECURITY:
   - employee_id comes ONLY from authenticated JWT/context.
   - browser does not supply a manager employee_id.
   - downstreamEmployeeHierarchyCTE is the source of truth.
   - because the shared CTE filters et.active = 'Yes',
     this endpoint returns ACTIVE downstream employees only.
...
func (
	...
	/* ========================================================
	   ...
	   Device count is aggregated first, which prevents asset
	   rows from duplicating employee rows.
	======================================================== */
		...
		employee_rows AS (
			SELECT
				...
				COALESCE(
					dc.device_count,
					0
				)::bigint AS device_count

			FROM downstream_employees de
	...
	case "direct":
		scopeFilter = `
			AND employee_rows.relationship = 'Direct'
		`

	case "indirect":
		scopeFilter = `
			AND employee_rows.relationship = 'Indirect'
		`
	}
	...
	if search != "" {
		...
		searchFilter = `
			AND (
				   COALESCE(employee_rows.employee_id, '') ILIKE $2
				OR COALESCE(employee_rows.employee_name, '') ILIKE $2
				OR COALESCE(employee_rows.designation, '') ILIKE $2
				OR COALESCE(employee_rows.department, '') ILIKE $2
				OR COALESCE(employee_rows.work_field, '') ILIKE $2
				OR COALESCE(employee_rows.sub_function, '') ILIKE $2
			)
			`
	}
	...
	listSQL :=
		fmt.Sprintf(
			...
			SELECT
				...
				employee_rows.employee_name,
				employee_rows.designation,
				employee_rows.department,
				employee_rows.work_field,
				employee_rows.sub_function,
				employee_rows.relationship,
				employee_rows.tier_level,
				employee_rows.device_count
			...
			ORDER BY
				CASE
					WHEN employee_rows.relationship = 'Direct'
						THEN 0
					ELSE 1
				END ASC,

				employee_rows.tier_level ASC,

				employee_rows.employee_name ASC,

				employee_rows.employee_id ASC
	...
	for rows.Next() {
		var item DownstreamEmployeeItem
