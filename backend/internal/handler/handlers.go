...
// 	// Kept separate from requisition approval/delivery permissions because
// 	// this action is initiated from the Trouble Ticket workflow.
// 	g.POST(
...
// 	// Trouble Ticket closing uses its own permission. Do not reuse TT_EDIT.
// 	// TT_Close should be assigned to ROOT, IT_ADMIN and IT_PERSONNEL.
// 	g.PATCH(
...
// // ─── Trouble Ticket Requisition / Close Workflow ───────────────────────────

// // RaiseTroubleTicketRequisition creates one active requisition reason record
// // for an open Trouble Ticket. The frontend sends a root inventory category ID,
// // an optional assigned-device serial, and the requisition reason.
// //
// // Permission is enforced by Register(): TT_REQUISITION.
...
// 	body.DeviceSerial = strings.TrimSpace(body.DeviceSerial)
// 	body.ReasonDetails = strings.TrimSpace(body.ReasonDetails)

// 	if body.CategoryID <= 0 {
// 		response.BadRequest(c, "device category is required")
// 		return
// 	}
// 	if body.ReasonDetails == "" {
// 		response.BadRequest(c, "reason details are required")
// 		return
// 	}
...
// 			COALESCE(status, ''),
// 			COALESCE(source_status, -1)
// 		FROM public.trouble_tickets
// 		WHERE id = $1
// 		FOR UPDATE
// 	`, ticketID).Scan(&ttNo, &employeeID, &ticketStatus, &sourceStatus)
// 	if err != nil {
// 		if err == pgx.ErrNoRows {
...
// 	if strings.EqualFold(strings.TrimSpace(ticketStatus), "closed") || sourceStatus == 0 {
// 		response.BadRequest(c, "a closed Trouble Ticket cannot create a requisition")
// 		return
// 	}
...
// 			LOWER(COALESCE(type, ''))
// 		FROM public.tbl_inventory_category
// 		WHERE id = $1
// 		  AND COALESCE(status, 1) = 1
// 	`, body.CategoryID).Scan(&category, &categoryParent, &categoryType)
// 	if err != nil {
// 		if err == pgx.ErrNoRows {
// 			response.BadRequest(c, "selected device category does not exist")
// 			return
// 		}
// 		response.ServerError(c, err)
// 		return
...
// 	if category == "" {
// 		response.BadRequest(c, "selected device category is invalid")
// 		return
// 	}

// 	// The requisition modal submits the top-level category. Reject a known
// 	// brand/model row here so accidental child IDs cannot be stored as category.
// 	if categoryType == "brand" || categoryType == "model" || categoryParent != 0 {
// 		response.BadRequest(c, "please select a top-level device category")
// 		return
// 	}

// 	// Prevent more than one active requisition for the same TT. Historical
// 	// inactive rows do not block creating a new one.
...
// 	if err == nil {
// 		response.BadRequest(c, "an active requisition already exists for this Trouble Ticket")
// 		return
// 	}
// 	if err != pgx.ErrNoRows {
// 		response.ServerError(c, err)
...
// 			source_requisition_date = CURRENT_TIMESTAMP,
...
// 		"tt_no":        ttNo,
// 		"category":     category,
// 		"device_sl_no": body.DeviceSerial,
// 		"status":       "Pending",
// 		"created_by":   createdBy,
// 	})
// }

// // CloseTroubleTicket closes an open Trouble Ticket and records the employee
// // who performed the close. It intentionally uses TT_Close instead of TT_EDIT.
// //
// // Permission is enforced by Register(): TT_Close.
...
// 	body.ClosingDescription = strings.TrimSpace(body.ClosingDescription)
// 	if body.ClosingDescription == "" {
// 		response.BadRequest(c, "closing description is required")
// 		return
// 	}

// 	closedBy := strings.TrimSpace(c.GetString("employee_id"))
// 	if closedBy == "" {
// 		response.BadRequest(c, "authenticated employee ID is missing")
// 		return
// 	}
...
// 		WHERE id = $1
// 		FOR UPDATE
// 	`, ticketID).Scan(&ttNo, &currentStatus, &sourceStatus)
// 	if err != nil {
// 		if err == pgx.ErrNoRows {
// 			response.NotFound(c, "Trouble Ticket not found")
// 			return
...
// 	if strings.EqualFold(strings.TrimSpace(currentStatus), "closed") || sourceStatus == 0 {
// 		response.BadRequest(c, "Trouble Ticket is already closed")
// 		return
// 	}
...
// 			updated_at = CURRENT_TIMESTAMP
// 		WHERE id = $1
// 	`, ticketID, closedBy, body.ClosingDescription)
// 	if err != nil {
// 		response.ServerError(c, err)
// 		return
// 	}
...
// 		"status":              "Closed",
// 		"closed_by":           closedBy,
// 		"closing_description": body.ClosingDescription,
// 	})
// }
