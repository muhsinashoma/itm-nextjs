...
func (h *ClaimHandler) ActiveByDevice(c *gin.Context) {
	...
	err := h.db.QueryRow(c.Request.Context(), `
        SELECT c.id, COALESCE(c.claim_no,'LEG-'||c.id::text), COALESCE(c.device_sl_no,''),
               ...
               COALESCE(c.lifecycle_state, CASE WHEN c.claim_status IN (8,15) THEN 'OPEN' WHEN c.claim_status=9 THEN 'WITH_VENDOR' WHEN c.claim_status=10 THEN 'CLOSED' ELSE 'LEGACY' END),
               c.restore_asset_status, c.restore_emp_id,
               COALESCE(c.vendor_id, c.vendor::bigint), v.vendor_name, c.created_at::text,
               COALESCE(c.service_type,0)
        ...
        LIMIT 1`, assetID).Scan(&id, &claimNo, &serial, &problem, &remarks, &claimStatus, &lifecycle, &restoreStatus, &restoreEmp, &vendorID, &vendorName, &createdAt, &serviceType)
	...
	response.OK(c, gin.H{"active": true, "claim": gin.H{
		...
		"vendor_name": vendorName, "created_at": createdAt, "service_type": serviceType,
	}})
}
...
func (h *ClaimHandler) OpenForDevice(c *gin.Context) {
	...
	var req struct {
		...
		TargetAssetStatus int    `json:"target_asset_status"`
	}
	...
	// Server decides the opening status from service_type.
	// Warranty Claim  => 8
	// Service Request => 15
	isServiceRequest := req.ServiceType == 1

	openStatus := claimOpen
	serviceType := 0
	eventType := "CLAIM_OPENED"
	historyLabel := "Claim Raised"
	referencePrefix := "WC"

	if isServiceRequest {
		openStatus = serviceRequestOpen
		serviceType = 1
		eventType = "SERVICE_REQUEST_OPENED"
		historyLabel = "Service Request"
		referencePrefix = "SR"
	}
	...
	if assetStatus != 0 && assetStatus != 1 && assetStatus != 4 {
		if isServiceRequest {
			response.BadRequest(c, "Service Request can be opened only from Available, Assigned, or Returned status.")
		} else {
	...
	if exists > 0 {
		response.BadRequest(c, "This device already has an active Warranty Claim or Service Request.")
		return
	}
	...
	err = tx.QueryRow(
		...
		req.Problems,
		strings.TrimSpace(req.EmailTo),
		strings.TrimSpace(req.EmailCC),
		actor,
		serviceType,
		assetStatus,
		empID,
	...
	c.JSON(http.StatusCreated, gin.H{
		...
		"data": gin.H{
			"id":                   claimID,
			"claim_no":             claimNo,
			"claim_status":         openStatus,
			"asset_status":         openStatus,
			"service_type":         serviceType,
			"lifecycle_state":      "OPEN",
			"restore_asset_status": assetStatus,
		},
	})
}
...
func (h *ClaimHandler) SendToVendor(c *gin.Context) {
	...
	expectedOpenStatus := claimOpen
	workflowLabel := "Warranty Claim"
	assetWorkflowLabel := "Claim: With Vendor"
	if serviceType == 1 {
		expectedOpenStatus = serviceRequestOpen
		workflowLabel = "Service Request"
		assetWorkflowLabel = "Service Request: With Vendor"
	}

	if status != expectedOpenStatus || lifecycle != "OPEN" {
		response.BadRequest(
			c,
			"only an OPEN "+workflowLabel+" record can be transferred to the vendor",
		)
		return
	}
	...
	historyReason := fmt.Sprintf(
		"%s %s transferred to vendor %s. Receiver: %s (%s). Gate pass: %s.",
		workflowLabel,
		claimNo,
		vendorName,
		vendorReceiver,
...
func (h *ClaimHandler) WarrantyWorkflow(c *gin.Context) {
	...
	if status != claimWithVendor && lifecycle != "WITH_VENDOR" {
		response.BadRequest(c, "Claim / Service workflow is available only after Transfer To Vendor")
		return
	}

	openAssetStatus := claimOpen
	workflowName := "Warranty Claim"
	withVendorRawStatus := "Claim: With Vendor"
	closedRawStatus := "Warranty Claim Closed"
	if serviceType == 1 {
		openAssetStatus = serviceRequestOpen
		workflowName = "Service Request"
		withVendorRawStatus = "Service Request: With Vendor"
		closedRawStatus = "Service Request Closed"
	}
	...
	if targetStatus == "9" {
		...
		if err = appendClaimAssetHistory(
			...
			openAssetStatus,
			fmt.Sprintf(
				"%s workflow transferred to vendor recipient %s (%s): %s",
				workflowName,
				vendorReceiver,
				mobileDigits,
				feedback,
	...
	if err = appendClaimAssetHistory(
		...
		*restore,
		...
		openAssetStatus,
		fmt.Sprintf(
			"%s %s closed and device restored to %s. Vendor recipient: %s (%s) | IT feedback: %s",
			workflowName,
			claimNo,
			label,
			vendorReceiver,
...
func (h *ClaimHandler) ReceiveFromVendor(c *gin.Context) {
	...
	var status int
	var serviceType int
	err = tx.QueryRow(ctx, `SELECT asset_device_id,COALESCE(device_sl_no,''),claim_status,COALESCE(service_type,0) FROM public.device_claims WHERE id=$1 FOR UPDATE`, claimID).Scan(&assetID, &serial, &status, &serviceType)
	if err != nil {
		if err == pgx.ErrNoRows {
			response.NotFound(c, "claim not found")
		} else {
	...
	_, err = tx.Exec(ctx, `INSERT INTO public.device_claim_histories (claim_id,asset_device_id,claim_reference_no,device_sl_no,previous_status,current_status,remarks,created_by,created_at,status,service_type,event_type) VALUES ($1,$2::bigint,$2::bigint::text,$3,$4,9,$5,$6,NOW(),1,$7,'RECEIVED_FROM_VENDOR')`, claimID, assetID, serial, status, strings.TrimSpace(req.Remarks), actor, serviceType)
	if err != nil {
		response.ServerError(c, err)
		return
	}
	openAssetStatus := claimOpen
	receivedLabel := "Claim: Received from Vendor"
	receivedReason := "Warranty device received from vendor: "
	if serviceType == 1 {
		openAssetStatus = serviceRequestOpen
		receivedLabel = "Service Request: Received from Vendor"
		receivedReason = "Service device received from vendor: "
	}
	...
	if err = appendClaimAssetHistory(
		...
		receivedLabel,
		claimWithVendor,
		receivedReason+strings.TrimSpace(req.Remarks),
	); err != nil {
		response.ServerError(c, err)
		return
	}
...
func (h *ClaimHandler) CloseLifecycle(c *gin.Context) {
	...
	var serviceType int
	err = tx.QueryRow(ctx, `SELECT asset_device_id,COALESCE(device_sl_no,''),claim_status,restore_asset_status,restore_emp_id,COALESCE(service_type,0) FROM public.device_claims WHERE id=$1 FOR UPDATE`, claimID).Scan(&assetID, &serial, &status, &restore, &restoreEmp, &serviceType)
	if err != nil {
		if err == pgx.ErrNoRows {
			response.NotFound(c, "claim not found")
		} else {
	...
	openAssetStatus := claimOpen
	closedLabel := "Warranty Claim Closed"
	workflowName := "Warranty claim"
	if serviceType == 1 {
		openAssetStatus = serviceRequestOpen
		closedLabel = "Service Request Closed"
		workflowName = "Service Request"
	}
	...
	if err = appendClaimAssetHistory(
		...
		*restore,
		closedLabel,
		openAssetStatus,
		fmt.Sprintf("%s closed and device restored to %s: %s", workflowName, label, strings.TrimSpace(req.Resolution)),
	); err != nil {
		response.ServerError(c, err)
		return
	}
