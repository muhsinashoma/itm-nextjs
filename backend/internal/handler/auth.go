...
// 		 FROM users WHERE (username=$1 OR email=$1) LIMIT 1`, req.Username).
...
// 		return
// 	}
// 	h.db.Exec(ctx, `INSERT INTO login_logs (user_id, ip_address) VALUES ($1, $2::inet)`,
// 		empID, c.ClientIP())

// 	response.OK(c, gin.H{
// 		"token": token, "expires_in": 28800,
// 		"user_id": id, "employee_id": empID,
// 		"username": username, "full_name": fullName, "user_type": userType,
// 	})
// }

// func (h *AuthHandler) Refresh(c *gin.Context) {
// 	h2 := c.GetHeader("Authorization")
// 	if len(h2) < 8 { response.BadRequest(c, "missing token"); return }
// 	claims := &middleware.Claims{}
// 	p := jwt.NewParser(jwt.WithoutClaimsValidation())
// 	if _, err := p.ParseWithClaims(h2[7:], claims, func(t *jwt.Token) (any, error) {
// 		return []byte(h.cfg.JWTSecret), nil
// 	}); err != nil {
// 		response.BadRequest(c, "invalid token"); return
// 	}
// 	tok, err := h.makeToken(claims.UserID, claims.EmployeeID, claims.Username, claims.UserType, 8*time.Hour)
...
// 	claims := &middleware.Claims{
// 		UserID: uid, EmployeeID: empID, Username: username, UserType: role,
