...
//     latest_employee.emp_id
// ) AS emp_id,

// COALESCE(
//     NULLIF(BTRIM(ad.emp_name), ''),
//     latest_employee.emp_name
// ) AS emp_name,

// COALESCE(
//     NULLIF(BTRIM(ad.department), ''),
//     latest_employee.department
// ) AS department,

// COALESCE(
//     NULLIF(BTRIM(ad.designation), ''),
//     latest_employee.designation
// ) AS designation,
...
// LEFT JOIN LATERAL (
//     SELECT
//         NULLIF(BTRIM(h.emp_id), '') AS emp_id,
//         NULLIF(BTRIM(h.emp_name), '') AS emp_name,
//         NULLIF(BTRIM(h.department), '') AS department,
//         NULLIF(BTRIM(h.designation), '') AS designation
//     FROM public.asset_device_history h
//     WHERE
//         (
//             h.asset_device_id = ad.id

//             OR (
//                 NULLIF(BTRIM(h.device_serial), '') IS NOT NULL
//                 AND UPPER(
//                     REGEXP_REPLACE(
//                         BTRIM(h.device_serial),
//                         '[^A-Za-z0-9]+',
//                         '',
//                         'g'
//                     )
//                 ) = ad.device_serial_key
//             )
//         )
//         AND (
//             NULLIF(BTRIM(h.emp_id), '') IS NOT NULL
//             OR NULLIF(BTRIM(h.emp_name), '') IS NOT NULL
//             OR NULLIF(BTRIM(h.department), '') IS NOT NULL
//             OR NULLIF(BTRIM(h.designation), '') IS NOT NULL
//         )
//     ORDER BY
