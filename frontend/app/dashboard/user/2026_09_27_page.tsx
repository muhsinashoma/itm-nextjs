...
const PAGE_SIZE = 10;
const QUERY_CHART_PAGE_SIZE = 100;
const QUERY_CHART_MAX_PAGES = 100;

type QueryChartType = "bar" | "pie" | "line";
type QueryChartRange = "3m" | "2m" | "1m";
type QueryTrendPoint = Record<string, string | number> & { label: string };

const QUERY_CHART_COLORS = [
    "#818cf8",
    "#7dd3fc",
    "#6ee7b7",
    "#fcd34d",
...
function getRangeMonths(range: QueryChartRange): number {
    return range === "1m" ? 1 : range === "2m" ? 2 : 3;
}

function getRangeStart(range: QueryChartRange): Date {
...
export default function UserDashboardPage() {
    ...
    const [queryChartRows, setQueryChartRows] = useState<OwnTroubleTicketItem[]>([]);
    const [queryChartLoading, setQueryChartLoading] = useState(true);
    const [queryChartError, setQueryChartError] = useState("");

    const loadQueryChart = useCallback(async () => {
        try {
            setQueryChartLoading(true);
            setQueryChartError("");

            const allRows: OwnTroubleTicketItem[] = [];
            let chartPage = 1;
            let expectedTotal = 0;

            do {
                const response = await ownDashboardApi.troubleTickets({
                    page: chartPage,
                    limit: QUERY_CHART_PAGE_SIZE,
                    status: "all",
                });
                ...
                if (rows.length === 0 || allRows.length >= expectedTotal) break;
                chartPage += 1;
            } while (chartPage <= QUERY_CHART_MAX_PAGES);

            setQueryChartRows(allRows);
        } catch (reason) {
            setQueryChartRows([]);
            setQueryChartError(
                reason instanceof Error ? reason.message : "Unable to load query analytics."
            );
        } finally {
    ...
    const queryTypeData = Array.from(
        ...
        .map(([query_type, total]) => ({ query_type, total }))
        .sort((a, b) => b.total - a.total || a.query_type.localeCompare(b.query_type));

    const queryChartTotal = queryTypeData.reduce((total, item) => total + item.total, 0);
    const topTrendTypes = queryTypeData.slice(0, 5).map((item) => item.query_type);

    const queryTrendData = (() => {
        const buckets = new Map<string, QueryTrendPoint>();

        for (const row of filteredQueryChartRows) {
            const createdAt = new Date(row.created_at);
            const weekStart = getWeekStart(createdAt);
            const key = weekStart.toISOString();

            if (!buckets.has(key)) {
                const base: QueryTrendPoint = { label: formatWeekLabel(weekStart) };
                topTrendTypes.forEach((queryType) => { base[queryType] = 0; });
                buckets.set(key, base);
            }

            const queryType = (row.query_type ?? "").trim() || "Uncategorized";
            if (!topTrendTypes.includes(queryType)) continue;

            const point = buckets.get(key);
            if (point) point[queryType] = Number(point[queryType] ?? 0) + 1;
        }
    ...
    return (
        <div className="min-w-0 space-y-3 p-2 sm:p-3 lg:p-4">
            ...
            {/* ==================================================
                QUERY ANALYTICS CHART SWITCHER
            ================================================== */}
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        <select
                            value={queryChartRange}
                            onChange={(event) =>
                                setQueryChartRange(
                                    event.target.value as QueryChartRange
                                )
                            }
                            className="h-8 rounded-lg border border-slate-200 bg-white px-2.5 text-[10px] font-semibold text-slate-700 outline-none transition focus:border-indigo-300"
                            aria-label="Query chart period"
                        >
                        ...
                        <select
                            value={queryChartType}
                            onChange={(event) =>
                                setQueryChartType(
                                    event.target.value as QueryChartType
                                )
                            }
                            className="h-8 rounded-lg border border-slate-200 bg-white px-2.5 text-[10px] font-semibold text-slate-700 outline-none transition focus:border-indigo-300"
                            aria-label="Query chart type"
                        >
                        ...
                        <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-right">
                            ...
                            <p className="text-sm font-bold tabular-nums text-slate-900">
                                {queryChartTotal.toLocaleString()}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-4">
                    {queryChartLoading ? (
                        ...
                        <div className="flex h-[360px] items-center justify-center rounded-xl border border-red-100 bg-red-50/50 px-4 text-center text-xs text-red-700">
                            {queryChartError}
                        </div>
                    ) : queryTypeData.length === 0 ? (
                        <div className="flex h-[360px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/40 text-center">
                            <Ticket className="h-7 w-7 text-slate-300" />
                            <p className="mt-2 text-xs font-semibold text-slate-700">
                                No query data found
                            </p>
                            <p className="mt-1 text-[10px] text-slate-500">
                                No tickets were created during the selected period.
                            </p>
                        </div>
                    ) : (
                        <div className="h-[360px] w-full">
                            ...
                            {queryChartType === "pie" && (
                                <div className="grid h-full gap-3 lg:grid-cols-[minmax(0,1fr)_220px]">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie
                                                data={queryTypeData}
                                    ...
                                    <div className="max-h-[320px] space-y-1 overflow-y-auto border-l border-slate-100 pl-3">
                                        {queryTypeData.map((item, index) => (
                                            <div
                                                key={`${item.query_type}-legend-${index}`}
                                            ...
                                            >
                                                <div className="flex min-w-0 items-center gap-2">
                                                    ...
                                                    />
                                                    <span
                                                        className="truncate text-[9.5px] font-medium text-slate-600"
