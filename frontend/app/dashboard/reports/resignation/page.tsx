...
function DeliveryStatusCell({
    ...
    const approvedVal = Number(
        item.approved_val ?? 0
    );

    const deliveredVal = Number(
        item.delivered_val ?? 0
    );

    const assignedVal = Number(
        item.device_assigned_val ?? 0
    );
    ...
    if (assignedVal !== 1) {
        return (
            <span className="inline-flex items-center gap-1 whitespace-nowrap rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-[8px] font-semibold text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/20 dark:text-amber-400">
    ...
    return (
        <span className="inline-flex items-center gap-1 whitespace-nowrap rounded-md border border-sky-200 bg-sky-50 px-2 py-1 text-[8px] font-semibold text-sky-700 dark:border-sky-900/60 dark:bg-sky-950/20 dark:text-sky-400">
...
function AssetAllocationModal({
            ...
            try {
                ...
                setError(
                    reason instanceof Error
                        ? reason.message
                        : "Unable to load available stock"
                );
            } finally {
                if (mounted) {
                    setLoading(false);
    ...
    async function assign() {
        if (!selectedStockId) {
            setError(
                "Select an available stock item first."
            );
            return;
        }
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/45 p-4 backdrop-blur-[1px]">
            <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-border bg-background shadow-2xl">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-[9px] text-muted-foreground">
                                Only available stock matching the approved requisition category is shown.
                            </p>
                        </div>
                    </div>
                ...
                <div className="grid border-b border-border bg-muted/15 sm:grid-cols-4">
                    {[
                        ["TT No", item.tt_no],
                        [
                            "Employee",
                        ...
                        >
                            ...
                            <p className="mt-1 line-clamp-2 text-[9px] font-medium text-foreground">
                                {value || "—"}
                            </p>
                        </div>
                    ))}
                ...
                <div className="flex-1 overflow-y-auto p-5">
                    <div className="mb-3 flex items-center gap-2 rounded-lg border border-border px-3">
                        ...
                        <input
                            ...
                            placeholder="Search serial, asset tag, brand, model, MR or PR..."
                    ...
                    {loading ? (
                        ...
                        <div className="flex min-h-[220px] flex-col items-center justify-center rounded-xl border border-dashed border-border text-center">
                            <Boxes className="mb-2 h-7 w-7 text-muted-foreground/50" />
                            <p className="text-[10px] font-semibold text-foreground">
                                No matching stock available
                            </p>
                            <p className="mt-1 max-w-md text-[9px] leading-5 text-muted-foreground">
                                Receive the item from SCM in Stock Entry first, or verify that the ITM category matches this requisition.
                            </p>
                        </div>
                    ) : (
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="mt-3 grid grid-cols-2 gap-2 text-[8px] text-muted-foreground">
                                            ...
                                            <span>
                                                Warranty: {asset.warranty_date ? formatDateTime(asset.warranty_date) : "—"}
                                            </span>
                                        </div>
                                    </button>
                                );
                    ...
                    {selectedStock && (
                        <label className="mt-4 block">
                            ...
                            <textarea
                                ...
                                placeholder="Optional: handover preparation, configuration or allocation note..."
                ...
                <div className="flex items-center justify-between gap-3 border-t border-border bg-muted/20 px-5 py-3">
                    <p className="text-[8px] text-muted-foreground">
                        Assignment is transactional: stock, requisition and asset registry update together.
                    </p>
...
function DeliveryConfirmModal({
    ...
    return (
        <div className="fixed inset-0 z-[115] flex items-center justify-center bg-black/45 p-4 backdrop-blur-[1px]">
            <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-background shadow-2xl">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <h2 className="text-sm font-semibold text-foreground">
                                Confirm Asset Handover
                            </h2>
                            <p className="mt-1 text-[9px] text-muted-foreground">
                                Delivery is confirmed only after an asset has been allocated.
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="space-y-4 p-5">
                    <div className="grid gap-3 rounded-xl border border-border bg-muted/20 p-3 sm:grid-cols-2">
                        <DetailField label="TT No" value={item.tt_no} />
                        <DetailField label="Employee" value={`${item.employee_id} • ${item.employee_name}`} />
                        <DetailField label="Asset" value={item.device_sl_no} />
                    ...
                    <label className="block">
                        ...
                        <textarea
                            ...
                            placeholder="Optional: receiver acknowledgement, configuration or delivery note..."
                            className="w-full resize-y rounded-lg border border-border bg-background px-3 py-2 text-[9px] outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10"
                        />
                    </label>
                </div>
                ...
                <div className="flex justify-end gap-2 border-t border-border bg-muted/20 px-5 py-3">
                    ...
                    <Button type="button" size="sm" disabled={saving} className="gap-1.5" onClick={() => void confirm()}>
                        {saving ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <Truck className="h-3.5 w-3.5" />}
                        Confirm Delivery
                    </Button>
                </div>
            </div>
                ...
                <div className="max-h-[calc(90vh-72px)] overflow-y-auto p-5">
                    ...
                    <div className="mb-4 flex flex-wrap items-center gap-4 rounded-lg border border-border bg-muted/20 px-3 py-2">
                        ...
                        <div>
                            ...
                            {Number(item.device_assigned_val ?? 0) === 1 ? (
                                <span className="inline-flex items-center gap-1 text-[9px] font-semibold text-sky-600 dark:text-sky-400">
                                    <PackageCheck className="h-3 w-3" />
                                    {item.device_sl_no || "Assigned"}
                                </span>
                            ) : item.approved_val === 1 || item.approved_val === 3 ? (
                                                                ...
                                                                <td className="px-2 py-2 text-center">
                                                                    <div className="flex flex-wrap items-center justify-center gap-1">
                                                                        <button
                                                                            ...
                                                                            className="inline-flex items-center gap-1 rounded border border-border bg-background px-2 py-1 text-[8px] font-semibold text-foreground transition-colors hover:bg-muted"
                                                                        >
                                                                        ...
                                                                        {(item.approved_val === 1 || item.approved_val === 3) &&
                                                                            Number(item.device_assigned_val ?? 0) === 0 &&
                                                                            Number(item.delivered_val ?? 0) === 0 && (
                                                                                <button
                                                                                    type="button"
                                                                                    onClick={() =>
                                                                                        setAllocationItem(
                                                                                    ...
                                                                                    className="inline-flex items-center gap-1 rounded border border-primary/30 bg-primary/5 px-2 py-1 text-[8px] font-semibold text-primary transition-colors hover:bg-primary hover:text-primary-foreground"
                                                                                >
                                                                        ...
                                                                        {(item.approved_val === 1 || item.approved_val === 3) &&
                                                                            ...
                                                                            Number(item.delivered_val ?? 0) === 0 && (
                                                                                <button
                                                                                    ...
                                                                                    className="inline-flex items-center gap-1 rounded border border-emerald-300 bg-emerald-50 px-2 py-1 text-[8px] font-semibold text-emerald-700 transition-colors hover:bg-emerald-600 hover:text-white"
                                                                                >
