...
export interface TroubleTicketAssignmentEvent {
    id: number;
    event_uuid: string;
    trouble_ticket_id: number;
...
export const ticketApi = {
    ...
    //2nd step attachment upload for bulk tickets
    createBulk: (
        ...
        // One file belongs to one ticket
        attachments.forEach(
            (file, index) => {
                if (!file) {
                    return;
    ...
    assignmentHistory: (
        ...
        api.get<
            ApiOk<TroubleTicketAssignmentEvent[]>
        >(
            `/tickets/${id}/assignment-history`
        ),
};
...
/* ======================================================
   ...
   SCM -> Stock -> Approved Requisition -> Asset -> Delivery
====================================================== */
...
export const inventoryWorkflowApi = {
    previewMR: (mr: string) =>
        api.get<ApiOk<SCMStockPreview>>(
            `/inventory-workflow/scm/mr/${encodeURIComponent(mr)}`
        ),

    importMR: (
        mr_id: string,
        items: SCMStockImportItem[]
    ) =>
        api.post<ApiOk<{
            mr_id: string;
            imported: number;
    ...
    availableStock: (params?: {
        ...
        api.get<ApiPage<AvailableStockItem>>(
            `/inventory-workflow/available-stock${toQuery(params)}`
        ),

    allocatableRequisitions: (params?: {
        category?: string;
        search?: string;
    }) =>
        api.get<ApiOk<AllocatableRequisition[]>>(
            `/inventory-workflow/allocatable-requisitions${toQuery(params)}`
        ),
    ...
    confirmDelivery: (
        ...
        api.post<ApiOk<{
            ...
            `/inventory-workflow/requisitions/${requisitionId}/deliver`,
            { remarks: remarks ?? "" }
        ),
};
