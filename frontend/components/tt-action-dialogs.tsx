...
export function TroubleTicketRequisitionDialog({
    ...
    }, [open, section?.id, section?.employee_id]);
...
export function TroubleTicketCloseDialog({
    ...
    useEffect(() => {
        ...
        // Warm the target route while the operator completes the form.
        router.prefetch(CLOSE_SUCCESS_PATH);
        setClosingDescription("");
        setError("");
    }, [open, section?.id, router]);
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            /*
             * Client-side navigation avoids the full browser reload caused
             * by window.location.assign(). The Closed Today route was
             * prefetched when the dialog opened for a smoother transition.
             */
            router.replace(CLOSE_SUCCESS_PATH);
        } catch (reason) {
